...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem('pulseops_token', result.accessToken);
    ...
    <div className="min-h-screen bg-pulseops-bg flex items-center justify-center px-4">
      ...
      >
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-heading font-bold text-pulseops-text">Welcome back</h1>
        ...
        <div className="bg-pulseops-surface border border-pulseops-border rounded-2xl p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-pulseops-text mb-1.5">Password</label>
            ...
            <div className="flex items-center justify-end">
              <Link href="/auth/forgot-password" className="text-xs text-pulseops-cyan hover:text-pulseops-cyan/80 transition-colors">
                Forgot password?
              </Link>
            </div>
            <button
          ...
          <div className="relative my-6">
            <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-pulseops-border" /></div>
            <div className="relative flex justify-center"><span className="px-3 text-xs text-pulseops-muted bg-pulseops-surface">Or continue with</span></div>
          </div>

          <div className="space-y-3">
            <a
              href={`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001').trim()}/api/auth/google`}
              className="flex items-center justify-center gap-2 w-full py-2.5 border border-pulseops-border rounded-xl text-sm font-medium text-pulseops-text hover:bg-pulseops-border/50 transition-colors"
            >
            ...
            <a
              href={`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001').trim()}/api/auth/github`}
              className="flex items-center justify-center gap-2 w-full py-2.5 border border-pulseops-border rounded-xl text-sm font-medium text-pulseops-text hover:bg-pulseops-border/50 transition-colors"
            >
              <Github size={18} />
              GitHub
        ...
        <p className="text-center mt-6 text-sm text-pulseops-muted">
          ...
          <Link href="/auth/signup" className="text-pulseops-cyan hover:text-pulseops-cyan/80 transition-colors font-medium">
            Sign up
          </Link>
        </p>
      </motion.div>
